...
def pad_collate_fn(samples: List[List[int]]):
    ...
    max_seq_len = max([len(b) for b in samples])
    padded_samples = [] # this will be our new list of lists of samples with 0s to pad
    for b in samples:
        sample = b.tolist() # this works and not list(b) otherwise we get a list of single tensor elements
        n_missing_zeros = max_seq_len - len(sample) # see how many zeros need to be added
        sample += [EOS_IX] # EOS before the padding to learn when to end the sentence
        sample += [PAD_IX] * n_missing_zeros # add them zeros
        padded_samples.append(sample)
    assert all(len(s) == max_seq_len+1 for s in padded_samples)
    t_p = torch.tensor(padded_samples) # this is a batch x length but we want a length by batch!
    # print(t_p.shape) 
    # print(t_p)
    return t_p.T
