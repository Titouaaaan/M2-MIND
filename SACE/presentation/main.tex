...
$F(\vec{x}, do(a, s)) \equiv \gamma_F^+(\vec{x}, a, s) \vee (F(\vec{x}, s) \wedge \neg \gamma_F^-(\vec{x}, a, s))$
        \textbf{Ex:} $Inside(emp,do(a,s)) \equiv \gamma_{Inside}^+(a,s) \vee (Inside(emp,s) \wedge \neg \gamma_{Inside}^-(a,s))$
\end{block}
    \pause
    \begin{block}{Plans}
...
\begin{frame}{Example}
    ...
    \begin{itemize}
        \item $onfire(s0) \wedge inside(emp, s0)$
        \item $Poss(call, s) \equiv \top$
        \item $Poss(extg, s) \equiv onfire(s)$
        \item $Poss(evac, s) \equiv inside(emp, s)$
    \end{itemize}
    
    Building is one fire, employees are inside, we can always call the fire department,
    we can only extinguish if there is a fire, and we can only evacuate if employees are inside.
...
\begin{frame}{Ethical Principles}
    ...
    \begin{block}{Hedonism}
        ...
        \{ \pi \in \Pi \mid goodness(\pi) > goodness(\emptyset)\}
        $$
    \end{block}
    \pause
    Example: \\
...
\begin{frame}{Ethical Principles}
    ...
    \pause
    Explanation:\\
    For every harmful predicate $\varphi$ (like $harmful(injured(emp)))$,
...
\underline{Note:} But there is a problem because lack of temporality (we will mention this later).
\end{frame}

\begin{frame}
    With the avoid-harm approach, we can define several harm-
ful fluents: $harmful(injured(emp)), harmful(injured'(emp))$.\\
Therefore, it is easy to see that No New Harm and Harm
Avoidance principles will always prioritise plans that avoid $injured(emp)$.\\
\vfill
\pause
\small
$\mathcal{EP}_{nnHarm}(\Pi, s_0) = \{[evac], [evac, call], [evac, extg], [evac, call, extg], [evac, extg, call]\}$\\
$\mathcal{EP}_{rHarm}(\Pi, s_0) = \Pi$\\
$\mathcal{EP}_{aHarm}(\Pi, s_0) = \{[evac], [evac, call], [evac, extg], [evac, call, extg], [evac, extg, call]\}$\\
\vfill
\pause
\tiny
\underline{Note:} Remove Harm returns all plans as ethically permissible because there is no existing harm in the initial state to remove. Do you see a problem with that?
\end{frame}
...
\begin{frame}{Definition}
    ...
    \vfill
    \pause
    \textbf{They don't decide permissibility but help us understand how a principle behaves.}
...
\begin{frame}{Ethical Properties}
    ...
    \end{block}
    if $\pi \in \mathcal{EP}(\Pi, s_0) \wedge \pi' \in \Pi$,  $goodness(\pi') \leq goodness(\pi)$
...
\begin{frame}{Future Directions and Impact}
    ...
    \begin{block}{Future Research Opportunities}
        \begin{itemize}
            \item Extend to temporal logic (big problem in my opinion).
            \item Study \textbf{prioritization of principles} in complex scenarios (with time!).
